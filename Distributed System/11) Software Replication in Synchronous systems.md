...
A consistency criterion de nes the result returned by an operation
- It de nes all acceptable runs.
Three main consistency criteria are de ned in literature
...
Replicas (the processes implementing $O$) **must agree** on the set of invocations they handle and **on the order** according to which they handle these invocations. (replica1 execute append, also replica2 execute append).

>Remember if i have two list with ame operations but different order, the list are not equal!
...
>If we respect these three properties, we garantuee that ll replica agree and execute in the same orders.

==Atomicity==
Atomicity: Given an invocation $[O op(arg) pi]$, if one replica of the object O handles this invocation, then every correct replica of O also handles the invocation $[O op(arg) pi]$. $\to$ **All or nothing**.
...
There is also another **problem**: if the client, after a $\Delta$ f time send another appen request to the primary, the List of the primary now contains two times the same value.
...
![[283.png]]
If there is another operations thath append after the first, this has timestamp equal to $1$.
...
![[284.png]]
To fix: Run leader election algorithm thath elect as a primary (leader) the process with the most updated state!
...
How to Guarantee atomicity? the update must be received either by all or by none of the backups. When a primary fails there is the need to elect another primary among the correct replicas. This primary has to be the one with the most updated copy of the
object. The primary has the job of making the backups on par before answering new requests.
...
#### Crash
does not need recovery actions upon the failure
of a replica
...
A replicated generic object (deterministic) needs consensus to be
implemented with linearisable semantic.
- Consensus is su cient to implement any generic replicated object.
