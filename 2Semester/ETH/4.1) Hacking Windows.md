...
- Main target: SMB, MSRPC, TS, SQL, SharePoint
- Automatic guessing on the CLI with tools like: `hydra, Venom, Brutus`
- Automatic guessing on GUI of terminal service/remote desktop Services like `Rdesktop and TSGrinder`
...
Cain: ARP Poisoning (reindirizzare il traffico di rete attraverso un attaccante. L'attaccante invia falsi messaggi ARP nella LAN, facendo credere ai dispositivi che il suo computer sia il gateway legittimo. Tutto il traffico passa attraverso l'attaccante, che può **sniffare dati, modificare pacchetti o rubare credenziali**.). Cain può forzare i client a usare **versioni deboli di autenticazione**

Cain può decifrare sessioni RDP su vecchie versioni windows a causa di:
1. Debolezze crittografiche
2. Mancanza di protezioni moderne
...
3. Dominio windows a rischio se attaccabte ottiene un account con privilegi sudo
...
È uno **strumento gratuito e molto usato** per **sfruttare vulnerabilità nei servizi di rete. nche se molto utile, **di solito è in ritardo di qualche mese** rispetto agli aggiornamenti di sicurezza pubblicati da Microsoft, quindi **non contiene gli exploit più recenti**.**

>Esistono alternative a pagamento (Core Impact e Canvas)

>Metasploit include anche exploit famosi, come quello per la **Print Spooler di Windows**, usato nel cyberattacco **Stuxnet** contro un reattore nucleare in Iran.
